<x-app-layout>
    ...
    <div class="py-8">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <form action="{{ route('merchant.services.update', $service) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                @csrf
                @method('PUT')
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <input type="text" id="name" name="name" value="{{ old('name', $service->name) }}" required class="w-full rounded-lg border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500">
                            @error('name') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                        </div>

                        <div>
                            <label for="description" class="block text-sm font-medium text-gray-700 mb-2">Description</label>
                            <textarea id="description" name="description" rows="5" required class="w-full rounded-lg border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500">{{ old('description', $service->description) }}</textarea>
                        ...
                        <div>
                            ...
                            <select name="currency" class="rounded-lg border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500 w-40">
                                @foreach(\App\Enums\Currency::cases() as $curr)
                                    <option value="{{ $curr->value }}" {{ old('currency', $service->currency?->value ?? config('app.currency')) === $curr->value ? 'selected' : '' }}>{{ $curr->value }} ({{ $curr->symbol() }})</option>
                                @endforeach
                            </select>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="number" id="price_from" name="price_from" value="{{ old('price_from', $service->price_from) }}" step="0.01" required class="w-full rounded-lg border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500">
                            ...
                            <div>
                                ...
                                <input type="number" id="price_to" name="price_to" value="{{ old('price_to', $service->price_to) }}" step="0.01" required class="w-full rounded-lg border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500">
                                @error('price_to') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                            </div>
                        </div>

                        <div class="flex items-center gap-3">
                            <input type="checkbox" id="is_negotiable" name="is_negotiable" value="1" {{ old('is_negotiable', $service->is_negotiable) ? 'checked' : '' }} class="rounded border-gray-300 text-amber-600 focus:ring-amber-500">
                        ...
                        <div>
                            ...
                            <input type="text" id="service_area" name="service_area" value="{{ old('service_area', $service->service_area) }}" placeholder="e.g., Lagos, Abuja, Nationwide" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <textarea id="highlights" name="highlights" rows="4" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500">{{ old('highlights', $service->highlights ? implode("\n", $service->highlights) : '') }}</textarea>
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-4 gap-3">
                                ...
                                <div class="relative">
                                    <img src="{{ Storage::url($image) }}" class="w-full h-24 object-cover rounded-lg" alt="Service image">
                                </div>
                                @endforeach
                            </div>
                        </div>
                ...
                <div class="flex justify-end gap-4">
                    <a href="{{ route('merchant.services.index') }}" class="px-6 py-3 bg-gray-100 text-gray-700 rounded-lg font-semibold hover:bg-gray-200 transition">Cancel</a>
                    <button type="submit" class="px-6 py-3 bg-amber-600 text-white rounded-lg font-semibold hover:bg-amber-700 transition">Update Service</button>
                </div>
            </form>
        </div>
    </div>
