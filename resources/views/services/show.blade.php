                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="bg-white border border-gray-200 rounded-2xl p-5">
                        ...
                        <div class="flex items-center gap-3 mb-4">
                            ...
                            @endif
                            <div>
                                <p class="font-semibold text-slate-900">{{ $service->merchant->isAdmin() ? 'Phantom 5 Merchant' : ($service->merchant->merchantProfile->business_name ?? $service->merchant->name) }}</p>
                                <p class="text-sm text-gray-500">{{ $service->merchant->isAdmin() ? 'Official Store' : 'Verified Service Provider' }}</p>
                            </div>
                        </div>
                        @php $servicePhone = $service->merchant->merchantProfile->business_phone ?? $service->merchant->phone ?? null; @endphp
                        @if($servicePhone)
                        <a href="tel:{{ $servicePhone }}" class="flex items-center justify-center gap-2 w-full bg-slate-800 text-white py-3 rounded-lg font-semibold hover:bg-slate-900 transition mb-3">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/></svg>
                            Call Provider
                        </a>
                        <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $servicePhone) }}?text={{ urlencode('Hi, I am interested in your service: ' . $service->name) }}" target="_blank" class="flex items-center justify-center gap-2 w-full bg-green-600 text-white py-3 rounded-lg font-semibold hover:bg-green-700 transition mb-3">
